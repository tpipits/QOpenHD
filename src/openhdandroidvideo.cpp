...
void OpenHDAndroidVideo::processFrame(QByteArray &nal) {
    if (!isConfigured) {
        return;
    }
...
void OpenHDAndroidVideo::renderLoop() {
    while (true) {
        ...
        auto status = AMediaCodec_dequeueOutputBuffer(codec, &info, 20000);
        if (status >= 0) {
            //AMediaCodec_releaseOutputBufferAtTime(codec, status, info.presentationTimeUs * 1000UL);
            //qDebug() << "AMediaCodec_releaseOutputBuffer";
